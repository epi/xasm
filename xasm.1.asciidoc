...
Atari 5200 mode is enabled with `opt g+` or `opt 'hardware=atari5200'`.
The abbreviations can also be rejected altogether with `opt 'hardware=none'`,
which is useful if the code targets neither of these machines.
...
The operand is either a sequence of single-letter switches
or <<new_optnames,a quoted list of `name=value` settings>>.
Six switches are available:
...
+
--
[[new_optnames]]Alternatively, the operand is a string in single or double quotes,
holding a comma-separated list of settings, each written as `name=value`:

------------------------------------------------------------------------------
 opt 'listing=off'
 opt 'output=rom,hardware=atari5200'
 opt "UNUSED_LABELS=Off"
------------------------------------------------------------------------------

The following settings are available:

- `hardware=atari800` - `^xx` are the hardware registers of Atari 8-bit computers
  (the default, same as `opt g-`)
- `hardware=atari5200` - `^xx` are the hardware registers of the Atari 5200
  (same as `opt g+`)
- `hardware=none` - `^xx` is rejected
- `listing=on` / `listing=off` - same as `opt l+` / `opt l-`
- `unused_labels=on` / `unused_labels=off` - same as `opt u+` / `opt u-`
- `output=ataridos` - write an Atari DOS executable file
  (the default, same as `opt f-h+o+`)
- `output=raw` - write the bytes with no headers (same as `opt f-h-o+`)
- `output=rom` - as above, but fill the space between noncontiguous memory areas
  with `$FF` bytes, which is what cartridge images need (same as `opt f+h-o+`)
- `output=off` - write no object file (same as `opt f-h-o-`)

Names and values are case-insensitive.
No spaces are allowed inside the string.
The empty string (`opt ''`) sets nothing.
An unknown name or an invalid value is an error.

`output` may be set only once in the source, and only before the first byte
of the object file is emitted. Once `output` is set, the `F`, `H` and `O`
...
- <<new_optnames,`OPT` alternatively accepts a quoted list of `name=value` settings>>,
  for example `opt 'output=rom,hardware=atari5200'`, including the new
  `output` format selection and `hardware=none` (by Adrian Matoga)
