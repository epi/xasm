...
Shorthand notation for Atari hardware registers is provided
...
This shorthand notation can be turned off with `opt ^-`
and turned back on with `opt ^+` (see below).
...
- `C` - accept <<new_65c02,65C02 instructions>>
- `F` - fill the space between noncontiguous memory areas with `$FF` bytes
- `G` - Atari 5200 mode for hardware register shorthand notation (`^xx`)
- `H` - generate Atari executable headers
- `L` - write the listing
- `O` - write the object file
- `U` - warn of unused labels
- `^` - enable hardware register shorthand notation (`^xx`)

+
You can turn any of these on or off.
The default (before the first `OPT`) is `opt c-f-g-h+l+o+u+^+`.
...
------------------------------------------------------------------------------
 ...
 opt c+     accept 65C02 instructions
 opt f+g+h- useful for Atari 5200 cartridges - raw output, 5200 hw regs
 opt ^-     disable hardware register shorthand notation (`^xx`)
...
[[new_65c02]]*xasm* assembles the standard 6502 (NMOS) instruction set by default.
`OPT C+` additionally enables the 65C02 instructions (as available on the
Atari Lynx console).
The Rockwell bit-manipulation instructions (`RMB`, `SMB`, `BBR`, `BBS`) and
power-saving instructions (`WAI`, `STP`) are not included.
`OPT C-` turns 65C02 mode back off.

The following new instructions become available:
...
|STZ        |store zero (`abs`, `zp`, `abs,x`, `zp,x`)
|TRB        |test and reset bits (`abs`, `zp`)
|TSB        |test and set bits (`abs`, `zp`)
|================================================

The following new addressing modes are added to existing instructions:

[cols="m,d",options="header"]
|====================================================================
|Example|Description
|inc @  |increment / decrement the accumulator (also `dec @`)
|lda ($80)|zero page indirect, for `ADC AND CMP EOR LDA ORA SBC STA`
|bit #$40|`BIT` with immediate, `zp,x` and `abs,x` addressing
|jmp ($1234,x)|absolute indexed indirect jump
|====================================================================
...
- *xasm* can now be used as a link:libxasm.html[library] in any D program
  (by Adrian Matoga)
- <<new_opt,`OPT ^-` disables the hardware register shorthand notation
  (`^xx`), `OPT ^+` re-enables it>> (by Adrian Matoga)
- <<new_65c02,`OPT C+` enables 65C02 instructions>> (by Adrian Matoga)
